...
direction TB
    ...
    note for 傷病名 "未コード病名（ワープロ病名）は考慮していない。"
    ...
    note for カルテ "予約ID : 予約時にオーダーを出すという要求を満たすのであれば必要になる。\nカルテなしでオーダーを出せるようにするのであればこの限りではない。"
    note for カルテ "ORCA連携であれば、ORCA側の保険を特定する情報を保持する必要がある。"
    ...
    note for SOAP "要求に合わせてひとまずSOAPとした。"
    カルテ "1" -- "1" SOAP
