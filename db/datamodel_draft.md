...
direction TB
    ...
    class 医師 {
        ...
        診療科ID
        麻薬施用者番号 : 外来では不要
        コメント("診療科ID : 診療所では複数になるのが一般的なので、配列または関連テーブル")
    }
    医療機関 "1" -- "1..n" 医師
    医師 "1" -- "1..n" 診療科
    ...
    class 患者["患者(スプリント1)"] {
        ...
        中分類("5-2.患者基本情報の表示")
        画面対応("5.患者の参照、6.患者情報編集・取り込み、19.患者基本情報、20.変更履歴")
    }
    患者 "1" -- "0..n" 受付
    患者 "1" -- "0..n" 予約

    class 患者基本情報["患者基本情報(スプリント1)"] {
        患者基本情報ID : PK
        患者ID : FK
        性別
        生年月日
        ...
        電話番号
        携帯番号
        コメント("「患者基本情報」として定義する項目を明確にするために切り出した")
        中分類("5-2.患者基本情報の表示")
        画面対応("5.患者の参照、6.患者情報編集・取り込み、19.患者基本情報、20.変更履歴")
    }
    患者基本情報 "1" -- "1" 患者

    class 血液型["血液型(スプリント1)"] {
        血液型ID : PK
        患者ID : FK
        血液型
        RH因子
    ...
    class アレルギー["アレルギー(スプリント1,6情報)"] {
        ...
        発症年月 : TODO 正規化するか
        中分類("5-3.患者プロファイルの表示")
        画面対応("23.食物アレルギー、24.その他アレルギー")
    }
    アレルギー "0..n" -- "1" 患者

    class 薬剤禁忌["薬剤禁忌(スプリント1,6情報)"] {
        薬剤禁忌ID : PK
        患者ID : FK
        薬剤禁忌コード : JFAGYコード
        長期保存フラグ
    ...
    class 感染症検体検査結果["感染症検体検査結果(スプリント1,6情報)"] {
        感染症検体検査結果ID : PK
        検査項目ID : FK
        標準検査マスターコード
        臨床検査会社・検査コード : nullable
    ...
    class 診療情報提供書["診療情報提供書(3文書α版スコープ)"] {
        診療情報提供書ID : PK
        医師ID : FK
        患者ID : FK
        提供先医療機関コード
        傷病名s : 診療情報提供書に含める傷病名のリスト
        既往歴s : 既往歴のリスト
        感染症検体検査結果s : 診療情報提供書に含める感染症検体検査結果のリスト
        アレルギーs : アレルギーのリスト
        家族歴s : 家族歴のリスト
        処方オーダーs : 診療情報提供書に含める処方オーダーのリスト
        ドキュメントs : 診療情報提供書に含めるドキュメントのリスト
        家族s : 親族のリスト
        発行日時
    }
    診療情報提供書 "0..n" -- "1" 患者
